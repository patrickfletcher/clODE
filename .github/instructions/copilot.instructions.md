...
- When a `.design/reference/` note exposes `## Fast path` or `## Bottom line`, read that first and stop unless the task needs deeper evidence.
...
- For `.design` maintenance or audit work, only pull in `.design/development_roadmap.md` when sequencing rationale itself is under review.
