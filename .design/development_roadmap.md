...
That strongest first proof target is now landed in the summary-observer family: `basic`, `basicall`, and `summary` now share one declaration family with explicit selection policy. The threshold-family follow-on proofs are now landed too: `Observer.threshold_crossing` proved that a useful absolute-threshold event family can keep threshold value and crossing direction in runtime settings while exposing only a lean timestamp-plus-count readout, `Observer.normalized_threshold_crossing` proved the same lean readout can coexist with warmup-derived normalized threshold parameterization, and the current Schmitt surface now spans both absolute and warmup-derived semantics through `Observer.schmitt_trigger` and `Observer.normalized_schmitt_trigger`, while `Observer.threshold_2` remains the retained legacy full normalized Schmitt path. The threshold-family naming and shared-config seam is now landed as well: the semantic observer names and the shared `ThresholdCrossingConfig`/`SchmittTriggerConfig` surfaces are preferred, while the broad parameter bundle and retained legacy `threshold_2` surface remain compatible. The next useful move is to keep that semantic threshold catalog stable, document the compatibility boundary around the semantic names and older bundles, and then decide whether recurring oscillation-oriented controls and outputs deserve a shared bundle seam instead of further family-local duplication.
