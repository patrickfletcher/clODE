...
That strongest first proof target is now landed in the summary-observer family: `basic`, `basicall`, and `summary` now share one declaration family with explicit selection policy. The first deterministic static-trigger counterexample is now landed too: `Observer.threshold_1` proves that a useful threshold event family can keep absolute threshold value and crossing direction in runtime settings while exposing only a lean timestamp-plus-count readout. The threshold-family naming and config seam is now landed as well: threshold crossing and Schmitt triggering have separate semantic names and separate config objects at the Python surface while the old pass-count names and broad parameter bundle remain compatible. The next useful move is therefore back on the implementation path: add the missing warmup-derived fractional directional-threshold family through that cleaner seam rather than through the generic threshold bundle.
