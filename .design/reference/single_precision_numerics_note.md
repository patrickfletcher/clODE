...
- Time bookkeeping and general value accumulation intentionally use different compensated-helper families: monotone elapsed-time state should use `compensatedTimeAdd(...)` and `compensatedTimeValue(...)`, while arbitrary sums and time-weighted integrals should use `compensatedSumAdd(...)` and `compensatedIntegrateConstant(...)`. Treat swapping those roles as a correctness bug, not as an equivalent implementation choice.
...
- If an observer or solver slice still needs persisted chunk-bridge elapsed state, update that state with the time-helper family rather than the general integral helper family.
