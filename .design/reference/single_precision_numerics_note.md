...
- `observer_threshold_2.clh` still records sampled up/down transition times in the live kernel; the new helper prototypes suggest inverse-linear interpolation is the most conservative first upgrade, while slope-aware Hermite interpolation should stay prototype-only until its robustness on noisy crossings is better characterized.
- `observer_local_maximum.clh` currently localizes events with a three-sample buffer and a sample-argmax choice; the new bounded quadratic max/min helpers suggest a shared utility upgrade is credible once component tests and a small observer-side adoption pass are scheduled.
