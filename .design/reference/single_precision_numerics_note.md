...
- The live fixed-step path reconstructs absolute time from `t0 + step * dt` with a step counter, while the live adaptive path accumulates solve-relative time with a compensated two-realtype elapsed pair and reconstructs stage and endpoint times from `t_origin + elapsed`.
...
- The helper-layer prototypes now support both live time paths cleanly: fixed-step kernels use `t0 + step * dt` from a 64-bit step counter, while adaptive kernels use a two-realtype compensated elapsed pair because there is no fixed-step counter shortcut.
...
- Keep the solver-owned dual-realtype compensated adaptive-time representation that is now live rather than reintroducing another single-float absolute-time variant.
