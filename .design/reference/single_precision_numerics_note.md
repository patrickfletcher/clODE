...
- `observer_normalized_schmitt_trigger.clh` uses inverse-linear timestamps for stored up/down threshold transitions in the live kernel. Slope-aware Hermite interpolation should still stay prototype-only until its robustness on noisy crossings is better characterized.
