...
- Phase 4-5: Planned; blocking dependencies explicit
...
- ✅ `docs/feature_extraction.md` updated — `ThresholdCrossingReadoutSelection` scoped as narrow pilot
- ✅ `ThresholdCrossingReadoutSelection` proof-of-concept — Validated as working pilot
...
**Goal**: Generalize `ThresholdCrossingReadoutSelection` pattern to other families and consolidate helper functions, based on Phase 4 decisions.
...
2. **Implement readout-selection types** for families with optional readouts:
   ...
   - `LocalExtremumReadoutSelection` (if selective readouts make sense for extrema)
   - `SchmittTriggerReadoutSelection` (if Phase 4 approves; would allow selecting period/amplitude/maxima count vs. Schmitt-specific readouts)
...
- The pilot `ThresholdCrossingReadoutSelection` is valid and will remain; it will either be generalized (Phase 5, if approved) or documented as example (if family-specific is final)
