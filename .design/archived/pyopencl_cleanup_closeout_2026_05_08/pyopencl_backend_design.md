...
- Related document: `.design/cpp_opencl_layer_audit.md`
- Forward-looking follow-up: `.design/pyopencl_post_migration_plan.md`
...
- the feature-backend work must not copy the legacy `ObserverData` byte-count formulas; `.design/pyopencl_struct_audit.md` records why device-aligned padding makes that unsafe in double precision
...
- `.design/pyopencl_rollout_guardrails.md` now records the rollout policy, the extended reference bundle, and the prerequisites for the eventual default switch
