...
- `matlab/`, `samples/`, `paper/`, `.design/`, and `.github/` are now outside the default source-distribution payload
...
The default source distribution now excludes several clearly non-runtime trees, including `.github/`, `paper/`, and `.design/`, in addition to the already-pruned stale `matlab/` and `samples/` trees.
