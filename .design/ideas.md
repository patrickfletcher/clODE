...
- [x] P0 Adopt the first evidence-backed live helpers: inverse-linear threshold timestamps, bounded three-sample local extrema, and fixed-step counter-reconstructed time where the current kernel/driver structure supports it cleanly. depends: current empirical demos and helper tests. refs: `clode/kernels/clODE_utilities.cl`, `clode/kernels/observers/`, `clode/kernels/steppers/`, `test/kernel_components/test_kernel_math.py`, `docs/numerical_accuracy.md`
- [ ] P1 Adaptive-time follow-on: implement a dual-realtype compensated time base plus relative elapsed bookkeeping for the observer paths that need it, without widening the public compatibility surface yet. depends: first live helper-adoption pass. refs: `clode/kernels/clODE_utilities.cl`, `clode/kernels/steppers/`, `clode/kernels/observers/`, `.design/reference/single_precision_numerics_note.md`
...
- [ ] P1 Slim exact-solution solver-validation suite: realistic ODE problems with global-error and convergence-rate checks in `test/core_numerics`, plus separate user-runnable examples/demos for work-precision diagrams and other numerical design investigations that stay outside the release gate. depends: representative problem set and stable numerics-bundle scope. refs: `test/core_numerics/`, `test/test_ornl_thompson_a1.py`, `examples/`, `docs/numerical_accuracy.md`, `.design/reference/testing_audit.md`
- [ ] P1 Examples or docs that show where compensated summation and related single-precision safeguards help in practice. refs: `examples/`, `docs/examples.md`, `docs/performance_notes.md`, `clode/kernels/clODE_utilities.cl`
