...
- [ ] P0 Explicit solver-state and cache ownership cleanup: give per-work-item solver state (`t0`, attained `tf`, `dt`, status flags, RNG continuation) a clear internal home distinct from IVP-owned problem data, persistent observer state, and fetched output caches, and reduce duplicated host-side mirrors between `Simulator` and `_opencl/executors.py`. blocks: continuation-policy helper, output-policy separation, stepper-definition cleanup, solver-state kernel abstractions, Random123 evaluation, implicit methods, observer-definition cleanup. refs: `.design/reference/continuation_timebase_note.md`, `clode/simulation/base.py`, `clode/_opencl/executors.py`, `clode/_opencl/buffers.py`
...
- [ ] P1 Python-owned stepper-definition model: explicit fixed/adaptive, explicit/implicit, deterministic/stochastic traits plus OpenCL mapping instead of splitting stepper semantics across `Stepper`, `_opencl.registry`, and kernel defines. depends: explicit solver-state model. blocks: cleaner implicit-stepper work and more intuitive source assembly. refs: `clode/simulation/base.py`, `clode/_opencl/registry.py`, `clode/kernels/steppers.cl`, `.design/reference/semantic_layout_audit.md`
- [ ] P1 Batch-generation helpers such as `grid`, random, and quasi-random sampling layered on top of the IVP model and current shape metadata instead of keeping ensemble creation buried in `Simulator`. depends: IVP-owned batch semantics. refs: `clode/simulation/base.py`, `.design/reference/semantic_layout_audit.md`
- [ ] P2 Execution-model experiments: `per-work-item` vs `per-work-group` vs shared/global solver state. depends: explicit solver-state model. refs: `clode/kernels/odedriver.cl`

## Observer And Feature Model

- [ ] P1 Explicit observer-definition model: params, persistent state, event storage, warmup, feature names. depends: explicit solver-state model and output/storage separation. blocks: custom/composable observers. refs: `clode/features.py`, `clode/observers/metadata.py`, `clode/_opencl/observer_metadata.py`, `clode/kernels/observers.cl`
- [ ] P1 Separate persistent observer state from optional event-output capacity. depends: explicit observer-definition model and output/storage separation. refs: `clode/_opencl/observer_metadata.py`, `clode/kernels/features.cl`, `.design/development_roadmap.md`
...
- [ ] P1 Ensemble batching for device-capacity limits. depends: IVP-owned batch semantics and integration/output separation. refs: `clode/trajectory.py`, `.design/development_roadmap.md`
