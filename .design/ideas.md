...
- [ ] P1 Public continuation-policy helper/API (`requested-window` vs attained-`tf` continuation) so callers do not have to hand-roll `set_tspan(get_final_time())`. depends: output/storage separation. refs: `docs/continuation.md`, `clode/simulation/base.py`
- [ ] P0 Separate integration state from output/storage policy (`max_store`, `nout`, event storage) so trajectory and feature allocation stop reading like solver state. blocks: chunked trajectory storage, observer-storage cleanup, public continuation helpers, ensemble batching. refs: `.design/next_pr.md`, `clode/simulation/params.py`, `clode/simulation/trajectory.py`, `clode/simulation/features.py`, `clode/_opencl/buffers.py`, `.design/reference/chunked_execution_audit.md`
- [ ] P1 Python-owned stepper-definition model: explicit fixed/adaptive, explicit/implicit, deterministic/stochastic traits plus OpenCL mapping instead of splitting stepper semantics across `Stepper`, `_opencl.registry`, and kernel defines. depends: output/storage separation and observer-definition cleanup. blocks: cleaner implicit-stepper work and more intuitive source assembly. refs: `clode/simulation/base.py`, `clode/_opencl/registry.py`, `clode/kernels/steppers.cl`, `.design/reference/semantic_layout_audit.md`
...
- [ ] P1 Explicit observer-definition model: params, persistent state, event storage, warmup, feature names. depends: output/storage separation. blocks: custom/composable observers. refs: `clode/features.py`, `clode/observers/metadata.py`, `clode/_opencl/observer_metadata.py`, `clode/kernels/observers.cl`
