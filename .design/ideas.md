...
- [ ] P0 Solver-owned per-work-item stepping diagnostics follow-through: extend the landed solver-owned status, step-count, last-accepted-step-width, and collapsed-window no-progress path to any remaining time-base values and any broader precision-loss status when solve time stops advancing in float32; keep only observer-private sample geometry where it is still required. depends: current wrapper-level status, step-count, and accepted-step-width plumbing. blocks: observer-state audit, cleaner continuation semantics. refs: `clode/simulation/_state.py`, `clode/_opencl/buffers.py`, `clode/_opencl/executors.py`, `clode/kernels/transient.cl`, `clode/kernels/features.cl`, `clode/kernels/trajectory.cl`, `clode/kernels/initializeObserver.cl`, `.design/reference/solver_state_implementation_plan.md`, `.design/reference/continuation_timebase_note.md`
...
- [ ] P2 Custom/composable observers from Python-authored definitions, including whether a snippet-driven Python-side model should generate the needed OpenCL code and buffer layout from pre-defined pieces instead of growing one hand-maintained observer catalog. depends: explicit observer-definition model and codegen story. refs: `clode/features.py`, `clode/function_converter.py`, `clode/kernels/observers.cl`
...
- [ ] P1 Observer-state and register-pressure audit for the heavier feature kernels: trim avoidable private scratch and unnecessary shared helper paths, quantify `max_event_timestamps` and per-observer state costs, and keep low-risk tidy-ups separate from deeper observer-time redesign. refs: `clode/kernels/observers/observer_local_maximum.clh`, `clode/kernels/observers/observer_neighborhood_1.clh`, `clode/kernels/observers/observer_neighborhood_2.clh`, `clode/kernels/observers/observer_threshold_2.clh`, `docs/performance_notes.md`
...
- [ ] P1 Slim exact-solution solver-validation suite: realistic ODE problems with global-error and convergence-rate checks in `test/core_numerics`, plus separate docs-backed user-runnable examples in `examples/` for work-precision diagrams and other numerical design investigations that stay outside the release gate; defer any broad top-level `benchmarks/` tree until the project commits to a stable workload matrix and non-gating hardware assumptions. depends: representative problem set and stable numerics-bundle scope. refs: `test/core_numerics/`, `test/test_ornl_thompson_a1.py`, `examples/`, `docs/numerical_accuracy.md`, `.design/reference/testing_audit.md`
