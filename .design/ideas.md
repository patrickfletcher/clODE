...
- [ ] P0 Public continuation-policy helper/API (`requested-window` vs attained-`tf` continuation) so callers do not have to hand-roll `set_tspan(get_final_time())`. refs: `docs/continuation.md`, `clode/simulation/base.py`, `.design/reference/continuation_timebase_note.md`
...
- [x] P0 Single source of truth for execution-setting defaults and compatibility resolution: constructor defaults and `SolverParams`/internal settings views now derive from one canonical helper so config choices stay predictable across transient, trajectory, and feature simulators, and simulators copy caller-provided `SolverParams` bundles instead of aliasing them. refs: `clode/simulation/params.py`, `clode/simulation/base.py`, `clode/simulation/trajectory.py`, `clode/simulation/features.py`, `test/test_simulation_contracts.py`
- [x] P0 Python-owned stepper-definition model: explicit fixed/adaptive, explicit/implicit, deterministic/stochastic traits plus OpenCL mapping now resolve through a Python-owned stepper-definition catalog instead of split registry tables. refs: `clode/simulation/_stepper_definitions.py`, `clode/_opencl/registry.py`, `clode/_opencl/source_builder.py`, `clode/_opencl/models.py`, `test/test_opencl_models.py`, `test/test_opencl_source_builder.py`
