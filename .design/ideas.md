...
- [ ] P1 Public continuation-policy helper/API (`requested-window` vs attained-`tf` continuation) so callers do not have to hand-roll `set_tspan(get_final_time())`. depends: stepper-definition cleanup. refs: `docs/continuation.md`, `clode/simulation/base.py`
...
- [ ] P0 Single source of truth for execution-setting defaults and compatibility resolution: constructor defaults and `SolverParams`/internal settings views should derive from one canonical source so config choices stay predictable across transient, trajectory, and feature simulators. depends: landed integration/output split and observer cleanup. blocks: stepper-definition cleanup and later public config redesign. refs: `clode/simulation/params.py`, `clode/simulation/base.py`, `clode/simulation/trajectory.py`, `clode/simulation/features.py`, `test/test_simulation_contracts.py`
- [ ] P1 Python-owned stepper-definition model: explicit fixed/adaptive, explicit/implicit, deterministic/stochastic traits plus OpenCL mapping instead of splitting stepper semantics across `Stepper`, `_opencl.registry`, and kernel defines. depends: execution-setting source-of-truth cleanup. blocks: cleaner implicit-stepper work, more intuitive source assembly, and later public config redesign. refs: `clode/simulation/base.py`, `clode/_opencl/registry.py`, `clode/kernels/steppers.cl`, `.design/reference/semantic_layout_audit.md`
...
- [x] P0 Explicit observer-definition model: params, persistent state, event storage, warmup, feature names, and build/invalidation semantics now resolve through `ObserverDefinition` and `ResolvedObserverSpec`. refs: `clode/observers/_definitions.py`, `clode/observers/metadata.py`, `clode/_opencl/observer_metadata.py`, `clode/_opencl/executors.py`, `clode/kernels/observers.cl`, `test/test_opencl_models.py`, `test/test_opencl_source_builder.py`
- [x] P1 Separate persistent observer state from optional event-output capacity. refs: `clode/_opencl/observer_metadata.py`, `clode/_opencl/executors.py`, `clode/kernels/features.cl`, `test/test_simulation_contracts.py`
...
- [ ] P2 Package/API hygiene: typing cleanup, stepper-specific parameter subsets, and trajectory output ergonomics once the execution-setting defaults are canonicalized. refs: `clode/solver.py`, `clode/features.py`, `clode/trajectory.py`
