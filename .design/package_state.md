...
- The active planning focus is the next solver-state follow-through: extend the now-landed status, accepted-step-count, last-accepted-step-width, and no-progress status path to any remaining time-base diagnostics and decide later whether a bundled solver-stats surface is worth freezing.
