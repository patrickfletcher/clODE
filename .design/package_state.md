...
- **Observer readout architecture**: canonical event observers expose event timestamps and event count plus trajectory summaries and oscillation-oriented aggregates (`period`/IMI, `n maxima`, `amplitude`) in observer-specific schemas. `local_max` remains the public extrema family with dual max/min event streams and IMI/amplitude outputs. Canonical threshold and Schmitt configs now both split trigger geometry on `event_var` from extrema/amplitude measurement on `feature_var`; threshold keeps its single-stream trigger semantics and `min_amp` gate on `event_var`, while Schmitt adds duration/duty and family-local `active dip` readouts. `normalized_neighborhood_return` likewise exposes `feature_var` on its semantic config and uses it for maxima/amplitude tracking while `event_var` still owns anchor geometry. `min_amp` semantics remain intentionally different between one-pass absolute and warmup-derived normalized families. Helper-hardening work (time/mean and interpolation utilities in `clODE_utilities.cl`) remains backlog.
