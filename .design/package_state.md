...
- The summary-observer family proof slice is now landed: `Observer.summary` plus `SummaryObserverSelection` specialize summary feature schemas and persistent state layouts to the requested subset, while `basic` and `basicall` remain compatibility presets over the same one-pass family. The threshold-family proof slices are now landed too: `Observer.threshold_crossing`, `Observer.normalized_threshold_crossing`, `Observer.schmitt_trigger`, and `Observer.normalized_schmitt_trigger` are the semantic threshold-family observers, while `Observer.threshold_2` remains the retained legacy fully featured normalized Schmitt path. `ThresholdCrossingConfig` is now shared by the two threshold-crossing families and `SchmittTriggerConfig` is shared by the two semantic Schmitt families plus the retained `threshold_2` path, while `ObserverParams` and the legacy `observer_*` keywords remain compatibility surfaces. The immediate architecture focus is now documenting that compatibility boundary before deciding whether recurring oscillation-oriented controls and readouts deserve a shared output-bundle seam.
