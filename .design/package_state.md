...
- The summary-observer family proof slice is now landed: `Observer.summary` plus `SummaryObserverSelection` specialize summary feature schemas and persistent state layouts to the requested subset, while `basic` and `basicall` remain compatibility presets over the same one-pass family. Event observers include `Observer.threshold_crossing`, `Observer.normalized_threshold_crossing`, `Observer.schmitt_trigger`, `Observer.normalized_schmitt_trigger`, `Observer.local_max` (with `Observer.local_extremum` as compatibility alias), and `Observer.neighborhood_return`, while `Observer.threshold_2` and `Observer.neighbourhood_2` remain retained heavier legacy workflows. `ThresholdCrossingConfig`, `SchmittTriggerConfig`, `LocalMaximumConfig` (with `LocalExtremumConfig` compatibility alias), and `NeighborhoodReturnConfig` are live semantic config surfaces; `ObserverParams` and legacy `observer_*` keywords remain compatibility adapters. Shared accepted-step history update helpers (`advanceAcceptedStepHistory2`, `advanceAcceptedStepHistory3`, and their `ByVariable` variants) are now live in `observers.cl` and consumed by all lean observer families.
...
- Public `SolverParams` still mixes integration policy with trajectory-output policy, `ObserverParams` still mixes runtime thresholds with event-output capacity at the compatibility surface, and `FeatureSimulator` still exposes legacy `observer_*` inputs alongside canonical `ObserverParams`. Threshold-family UX now makes that debt concrete: one-boundary threshold crossing and Schmitt triggering need different knob sets and different preferred names, and the selected observer rather than the shared config class now decides absolute versus warmup-derived interpretation.
...
- **Observer readout architecture** (Phase 4 complete): Summary trajectory statistics (`max`/`min`/`mean` per variable), period (`inter-event interval`), and amplitude (`max`/`min`/`mean`) are implemented across semantic event-detector families. `local_max` is canonical for local-extrema workflows, with one kernel implementation (`observer_local_maximum.clh`) and compatibility alias routing for `local_extremum`. Event-observer readouts are standardized around event times, event counts, optional event values, and trajectory statistics, with dual-stream naming for observers that expose two event lists (local maximum/minimum, up/down transition). Schmitt-specific readouts (up/down durations, duty cycle) remain family-local pending explicit parity decisions. `min_amp` semantics remain intentionally different between one-pass absolute and warmup-derived normalized families. A dedicated helper-hardening backlog remains for compensated-time/mean and interpolation routine standardization in `clODE_utilities.cl`. **Phase 5 forward**: summary observer family enhancement to support optional period and amplitude tracking remains deferred.
...
- Observer event and feature variable semantics (Phase 4): `.design/reference/observer_event_feature_variable_contract.md`
