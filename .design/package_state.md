...
- The summary-observer family proof slice is now landed: `Observer.summary` plus `SummaryObserverSelection` specialize summary feature schemas and persistent state layouts to the requested subset, while `basic` and `basicall` remain compatibility presets over the same one-pass family. The semantic event catalog now also includes `Observer.threshold_crossing`, `Observer.normalized_threshold_crossing`, `Observer.schmitt_trigger`, `Observer.normalized_schmitt_trigger`, `Observer.local_extremum`, and `Observer.neighborhood_return`, while `Observer.threshold_2`, `Observer.local_max`, and `Observer.neighbourhood_2` remain retained heavier legacy workflows. `ThresholdCrossingConfig`, `SchmittTriggerConfig`, `LocalExtremumConfig`, and `NeighborhoodReturnConfig` are now live at the semantic surface, while `ObserverParams` and the legacy `observer_*` keywords remain compatibility adapters. The immediate architecture focus is whether a shared accepted-step solution-buffer concept should come before further observer bundle work; the helper-utility layer currently looks mostly like private implementation support rather than the next standalone proof target.
