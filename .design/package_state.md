...
- The active planning focus is a summary-observer family proof target: use the completed observer concept audit to unify `basic` and `basicall` behind one smaller declaration model and prove finer-grained feature selection and state-footprint control before reopening the heavier event observers.
...
- The owner split is clearer than it was before the last refactor, but there is still no single reusable Python-side observer-authoring layer that packages persistent observer state, runtime settings, event-output policy, and fetched observer outputs into one easy-to-extend definition model.
- Observer selection granularity is still coarse: aside from `basic` plus `event_var` and `feature_var`, built-in observers mostly hard-code all-variable or fixed feature bundles, so observer-state footprint and output schema still follow the chosen mode more than the user's specific feature interests.
