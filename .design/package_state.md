...
- Simulation and executor layers now keep transient, feature, and trajectory results in derived caches with explicit invalidation across `t_span`, observer, and output-layout changes, and the OpenCL test layer now covers the main rebuild and build-key paths directly.
...
- Adaptive steppers now keep solve-relative compensated elapsed time inside the kernels, and the elapsed-time-sensitive built-in observers now accumulate means, periods, and durations from accepted-step relative elapsed bookkeeping instead of subtracting large float32 absolute timestamps.
- The current package still has a few cleanup targets, especially stronger validation evidence, observer-state and private-memory footprint follow-through in the heavier feature kernels, diverged-time continuation modeling, selective remaining helper adoption, and later chunking work on top of the landed IVP, solver-state, output-policy, observer, execution-setting, and stepper-definition boundaries.
...
- Fixed-step live steppers now reconstruct absolute time from `t0 + step * dt` with a local 64-bit step counter. Adaptive steppers now reconstruct time from `tspan[0]` plus a compensated solve-relative elapsed pair, but absolute float32 timestamps are still quantized by local spacing.
- The repo now has a dedicated public float32 demonstration path in `examples/single_precision_accuracy.py` plus `docs/numerical_accuracy.md`; the current live mitigation slice includes fixed-step counter time, compensated adaptive elapsed time, relative elapsed observer bookkeeping for affected means and durations, `threshold_2` upward timestamp refinement, and `local_max` three-sample extremum refinement, while broader remaining helper rollout is still intentionally selective.
...
- The dedicated kernel-component layer now exists, and it now covers helper math, large-origin elapsed-time prototypes, fixed-step versus compensated time prototypes, a direct adaptive-step large-origin contract, threshold-crossing interpolation helpers, duration-sensitive threshold checks, and three-sample extremum helpers in addition to direct `basic` and `basicall` observer contracts.
