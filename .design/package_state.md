...
- The summary-observer family proof slice is now landed: `Observer.summary` plus `SummaryObserverSelection` specialize summary feature schemas and persistent state layouts to the requested subset, while `basic` and `basicall` remain compatibility presets over the same one-pass family. A first deterministic static-trigger slice is now also landed: `Observer.threshold_1` adds one-pass absolute threshold crossings with runtime direction selection and a lean timestamp-plus-count readout. `Observer.threshold_2` remains the warmup-derived fractional Schmitt-style family, and the current planning focus is the next missing threshold corner: a lean warmup-derived directional-threshold family that keeps threshold crossing distinct from hysteresis.
