...
- **Observer readout architecture**: canonical event observers expose event timestamps and event count plus trajectory summaries and oscillation-oriented aggregates (`period`/IMI, `n maxima`, `amplitude`) in observer-specific schemas. `local_max` remains the public extrema family with dual max/min event streams and IMI/amplitude outputs. Canonical Schmitt schemas now also expose `up duration`, `down duration`, and `duty`, and they retain the family-local `active dip` readout; Schmitt maxima/amplitude/active-dip tracking follows `feature_var` while threshold geometry stays on `event_var`. `min_amp` semantics remain intentionally different between one-pass absolute and warmup-derived normalized families. Helper-hardening work (time/mean and interpolation utilities in `clODE_utilities.cl`) remains backlog.
