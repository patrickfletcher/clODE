...
- Whether to expose raw observer state directly, and whether `ObserverOutput` should remain the long-lived public name for the current feature or event readout object, is intentionally deferred until the owner split lands cleanly enough to compare alternatives without compatibility pressure.
