...
- The summary-observer family proof slice is now landed: `Observer.summary` plus `SummaryObserverSelection` specialize summary feature schemas and persistent state layouts to the requested subset, while `basic` and `basicall` remain compatibility presets over the same one-pass family. A first deterministic static-trigger slice is now also landed: `Observer.threshold_crossing` is the preferred semantic alias for the current one-pass absolute threshold observer, while `Observer.schmitt_trigger` is the preferred semantic alias for the current warmup-derived fractional Schmitt-style family. The older `threshold_1` and `threshold_2` spellings remain compatibility aliases. Threshold crossing and Schmitt triggering now also have separate family-specific config objects on the Python side, while `ObserverParams` and the legacy `observer_*` keywords remain compatibility surfaces. The immediate architecture focus is now the next missing threshold family: a lean warmup-derived directional-threshold follow-on that uses the landed config seam instead of extending the broad compatibility bundle again.
